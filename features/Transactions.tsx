        ...
        <Card>
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-3">
                ...
                <div className="flex gap-2 flex-wrap justify-center">
                    <button onClick={() => setIsImportModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2 px-4 rounded-lg text-sm">Impor</button>
                    ...
                    <button onClick={() => handleOpenModal()} className="bg-primary hover:bg-primary/80 text-white font-bold py-2 px-4 rounded-lg text-sm">+ Transaksi Baru</button>
                </div>
            </div>
                    ...
                    {paginatedTransactions.length > 0 ? paginatedTransactions.map(tx => (
                        <Card key={tx.id} className="!p-4">
                            ...
                            <div className="mt-3 pt-3 border-t border-slate-700 flex justify-end space-x-4">
                                <button onClick={() => handleOpenModal(tx)} className="text-blue-400 hover:underline text-sm">Edit</button>
                                <button onClick={() => handleDelete(tx.id!)} className="text-red-400 hover:underline text-sm">Hapus</button>
                            </div>
                        </Card>
                    )) : (
