...
const AssetForm: React.FC<{
    ...
    const accumulatedDepreciationAccounts = useMemo(() => accounts.filter(acc => acc.type === 'Aset' && acc.name.toLowerCase().includes("akum")).sort((a,b) => a.code.localeCompare(b.code)), [accounts]);
    const assetTypeAccounts = useMemo(() => accounts.filter(acc => acc.type === 'Aset' && !acc.name.toLowerCase().includes("akum")).sort((a,b) => a.code.localeCompare(b.code)), [accounts]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (assetToEdit?.id) {
                if (!canEdit) throw new Error("Anda tidak memiliki izin untuk mengedit aset.");
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} className={formInputClasses} required disabled={!canEdit && !!assetToEdit} />
                    </div>
                    <div>
                        <label htmlFor="category" className="block text-sm font-medium text-gray-300 mb-1">Kategori</label>
                        <input type="text" name="category" id="category" value={formData.category} onChange={handleInputChange} className={formInputClasses} placeholder="e.g., Peralatan Kantor" disabled={!canEdit && !!assetToEdit} />
                    </div>
                    <div>
                        <label htmlFor="date" className="block text-sm font-medium text-gray-300 mb-1">Tanggal Perolehan</label>
                        <input type="date" name="date" id="date" value={formData.date} onChange={handleInputChange} className={formInputClasses} required disabled={!canEdit && !!assetToEdit} />
                    </div>
                    <div>
                        <label htmlFor="cost" className="block text-sm font-medium text-gray-300 mb-1">Harga Perolehan</label>
                        <input type="number" name="cost" id="cost" value={formData.cost || ''} onChange={handleInputChange} className={formInputClasses} required disabled={!canEdit && !!assetToEdit} />
                    </div>
                    <div>
                        ...
                        <input type="number" name="accumulated_depreciation" id="accumulated_depreciation" value={formData.accumulated_depreciation || ''} onChange={handleInputChange} className={formInputClasses} disabled={!assetToEdit || !canEdit} />
                ...
                <div className="pt-4 border-t border-slate-700">
                    <label className="flex items-center space-x-2">
                        <input type="checkbox" name="is_depreciable" checked={formData.is_depreciable} onChange={handleInputChange} className="h-4 w-4 rounded bg-slate-700 border-slate-600 text-primary focus:ring-primary"  disabled={!canEdit && !!assetToEdit}/>
                    ...
                    <div className="space-y-4 p-4 bg-slate-900 rounded-lg">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="number" name="life" id="life" value={formData.life || ''} onChange={handleInputChange} className={formInputClasses} disabled={!canEdit && !!assetToEdit} />
                            </div>
                            <div>
                                <label htmlFor="residual" className="block text-sm font-medium text-gray-300 mb-1">Nilai Residu</label>
                                <input type="number" name="residual" id="residual" value={formData.residual || ''} onChange={handleInputChange} className={formInputClasses} disabled={!canEdit && !!assetToEdit} />
                            </div>
                            <div>
                                <label htmlFor="method" className="block text-sm font-medium text-gray-300 mb-1">Metode</label>
                                <select name="method" id="method" value={formData.method || ''} onChange={handleInputChange} className={formInputClasses} disabled={!canEdit && !!assetToEdit}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-700">
                             <div>
                                ...
                                <select name="depreciation_expense_account_code" id="depreciation_expense_account_code" value={formData.depreciation_expense_account_code || ''} onChange={handleInputChange} className={formInputClasses} required={formData.is_depreciable} disabled={!canEdit && !!assetToEdit}>
                             ...
                             <div>
                                ...
                                <select name="accumulated_depreciation_account_code" id="accumulated_depreciation_account_code" value={formData.accumulated_depreciation_account_code || ''} onChange={handleInputChange} className={formInputClasses} required={formData.is_depreciable} disabled={!canEdit && !!assetToEdit}>
                                    <option value="">Pilih akun...</option>
    ...
    return (
        <Card>
            <div className="flex justify-between items-center mb-6">
                ...
                {canCreate && <button onClick={() => handleOpenFormModal()} className="bg-primary hover:bg-primary/80 text-white font-bold py-2 px-4 rounded-lg">
                    + Aset Baru
                </button>}
            </div>
                                    ...
                                    return (
                                        <tr key={asset.id} className="border-b border-gray-700 hover:bg-slate-800/50">
                                            ...
                                            <td className="py-3 px-4 text-center space-x-2">
                                                {asset.is_depreciable && canCreate && <button onClick={() => handleOpenDepreciationModal(asset)} className="text-accent hover:underline text-sm">Penyusutan</button>}
                                                <button onClick={() => handleOpenFormModal(asset)} className="text-blue-400 hover:underline text-sm">{canEdit ? 'Edit' : 'Lihat'}</button>
                                                {canDelete && <button onClick={() => handleDelete(asset.id!)} className="text-red-400 hover:underline text-sm">Hapus</button>}
                                            </td>
                                        </tr>
                                    );
                                })
            ...
            {canCreate && <DepreciationModal isOpen={isDepreciationModalOpen} onClose={() => setIsDepreciationModalOpen(false)} asset={depreciationAsset} />}
        </Card>
    );
};
