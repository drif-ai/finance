...
export const calculatePeriodFinancials = (accounts: Account[], transactions: Transaction[], period: { start: string, end: string }) => {
    ...
    // 1. Calculate closing balances (cumulative balances as of period end)
    const closingBalances = new Map<string, number>(accounts.map(a => [a.code, 0]));
    transactions
        .filter(tx => tx.date <= period.end)
        .forEach(tx => {
            tx.entries.forEach(e => {
                const change = e.debit - e.credit;
                closingBalances.set(e.account_code, (closingBalances.get(e.account_code) || 0) + change);
            });
        });

    // 2. Calculate opening balances (cumulative balances as of the day before period start)
    ...
    // 3. Calculate Income Statement figures (based on transactions ONLY within the period)
    ...
    const netIncome = totalRevenue - totalExpense; // Net income for the specific period

    // 4. Calculate Balance Sheet figures (using closing balances)
    ...
    // To ensure the Balance Sheet balances, we must include the cumulative profit/loss from all P&L accounts up to the report's end date.
    // This represents the "current earnings" that are part of equity.
    const cumulativePL = -accounts
        .filter(a => a.type === 'Pendapatan' || a.type === 'Beban')
        .reduce((sum, acc) => sum + (closingBalances.get(acc.code) || 0), 0);
        
    // Add this cumulative P/L to the Retained Earnings account ('3200') for reporting purposes.
    const equityWithPL = equity.map(a => a.code === '3200' ? {...a, balance: a.balance + cumulativePL} : a);

    const totalAssets = assets.reduce((s, a) => s + (a.name.toLowerCase().includes('akumulasi') ? -(a.balance) : a.balance), 0);
    const totalLiabilities = liabilities.reduce((s, a) => s + a.balance, 0);
    const totalEquity = equity.reduce((s, a) => s + a.balance, 0); // This is equity before adding cumulative PL
    ...
    // 5. Calculate Cash Flow (using opening and closing balances of cash accounts)
